    ...
    <div className="w-full min-h-screen bg-gray-50/50 p-4 md:p-8 animate-in fade-in duration-500">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="flex items-center gap-4 mb-10">
          <Link href={`/linkedin/${accountId}/campaigns`} className="p-3 bg-white border border-gray-100 rounded-2xl text-gray-400 hover:text-gray-900 transition-all shadow-sm">
            <ChevronLeft size={20} />
          </Link>
        ...
        <form onSubmit={handleSubmit} className="space-y-10">
          ...
          <div className="bg-white rounded-[2.5rem] border border-gray-100 shadow-xl shadow-gray-200/20 overflow-hidden">
            <div className="p-6 md:p-8 border-b border-gray-50 flex items-center gap-4 bg-gray-50/30">
              <div className="w-12 h-12 bg-white rounded-2xl flex items-center justify-center shadow-sm text-[#0077B5]">
                <Target size={24} />
              </div>
              <div>
            ...
            <div className="p-8 md:p-10 grid grid-cols-1 md:grid-cols-2 gap-8">
              <div className="space-y-3">
                ...
                <input
                  ...
                  placeholder="e.g. CEO Outreach v1"
                  className="w-full bg-gray-50 border border-gray-100 rounded-2xl px-6 py-4 outline-none focus:ring-4 focus:ring-blue-500/10 focus:border-blue-500/30 focus:bg-white transition-all font-bold text-gray-900 placeholder-gray-300"
                  required
                />
              </div>
              <div className="space-y-3">
            ...
            <div className="p-8 md:p-10 space-y-8">
              ...
              <div className="space-y-4">
                 <div className="flex items-center justify-between ml-1">
                    <label className="text-[10px] font-black text-gray-400 uppercase tracking-[0.2em]">Connection Note (Optional)</label>
                    <span className="text-[9px] font-black text-gray-400">{connectionNote.length}/300</span>
                 </div>
                 <div className="flex items-center gap-2 mb-2">
                    {["$$f_name$$", "$$full_name$$"].map(v => (
                      <button key={v} type="button" onClick={() => insertVariable("note", v)} className="px-3 py-1 bg-white border border-gray-100 rounded-lg text-[9px] font-black text-[#0077B5] hover:border-blue-200 transition-all">
                        {v}
                      </button>
                    ))}
                 </div>
            ...
            <div className="p-8 md:p-10 space-y-8">
              <div className="flex flex-col md:flex-row gap-6 items-start md:items-center">
                 <div className="flex-1 w-full space-y-3">
                    ...
                    <select
                      ...
                      className="w-full bg-sky-50/50 border border-sky-100 rounded-2xl px-6 py-4 outline-none focus:ring-4 focus:ring-blue-500/10 transition-all font-bold text-blue-900 appearance-none text-sm"
                    >
                 ...
                 {!showSaveTemplate ? (
                    <button
                      ...
                      className="whitespace-nowrap flex items-center gap-2 text-[#0077B5] font-black text-[10px] uppercase tracking-widest hover:bg-sky-50 px-6 py-4 rounded-2xl border border-dashed border-sky-200 transition-all mt-6 md:mt-7"
                    >
                      <Plus size={16} /> Save as New Template
                    </button>
                 ) : (
                    <div className="flex-1 w-full p-6 bg-sky-50/30 rounded-3xl border border-sky-50 flex flex-col gap-4 shadow-inner">
                       <input
                        type="text"
                        value={templateName}
                        onChange={(e) => setTemplateName(e.target.value)}
                        placeholder="Template Identifier"
                        className="w-full bg-white border border-sky-100 rounded-xl px-4 py-2 text-sm font-bold outline-none"
                      />
                      <div className="flex gap-2">
                        <button onClick={handleSaveAsTemplate} className="flex-1 bg-[#0077B5] text-white text-[9px] font-black uppercase py-2 rounded-lg">Construct Template</button>
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                   <label className="text-[10px] font-black text-gray-400 uppercase tracking-[0.2em] ml-1">Main Sequence Payload</label>
                   <div className="flex items-center gap-2">
                      {["$$f_name$$", "$$full_name$$", "$$company$$"].map(v => (
                        <button
                          ...
                          className="px-3 py-1 bg-white border border-gray-100 rounded-lg text-[9px] font-black text-[#0077B5] hover:border-sky-200 hover:bg-sky-50 transition-all"
                        >
                          {v}
                        </button>
                      ))}
            ...
            <div className="p-8 md:p-10 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
              <div className="space-y-3">
                ...
                <div className="relative">
                  ...
                  />
                  <span className="absolute right-4 top-1/2 -translate-y-1/2 text-[9px] font-black text-amber-500 uppercase">Max 40 Safe Range</span>
                </div>
              </div>
              <div className="space-y-3">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <input type="number" max="180" value={maxDelay} onChange={(e) => setMaxDelay(e.target.value)} className="w-full bg-gray-50 border border-gray-100 rounded-xl p-3 text-center font-bold" />
                </div>
              </div>
               <div className="space-y-3">
              ...
              {followUps.length === 0 ? (
                ...
                <div className="space-y-6">
                  {followUps.map((step, idx) => (
                    <div key={idx} className="bg-gray-50/50 rounded-3xl border border-gray-100 p-8 relative group hover:bg-white hover:border-sky-100 transition-all shadow-sm">
                       <button
                        type="button"
          ...
          <div className="bg-[#0A1622] rounded-[2.5rem] border border-sky-900/30 shadow-2xl p-8 md:p-10 flex flex-col md:flex-row items-center justify-between gap-10">
             <div className="flex-1">
                ...
                <div className="flex items-center justify-between p-6 bg-sky-950/20 rounded-3xl border border-sky-900/30">
                   ...
                   <label className="relative inline-flex items-center cursor-pointer">
                    ...
                    />
                    <div className="w-14 h-8 bg-sky-900/40 peer-focus:outline-none rounded-full peer peer-checked:after:translate-x-6 after:content-[''] after:absolute after:top-[4px] after:left-[4px] after:bg-white after:rounded-full after:h-6 after:w-6 after:transition-all peer-checked:bg-[#0077B5] border border-sky-800 transition-all shadow-inner"></div>
                  </label>
                </div>
             </div>
