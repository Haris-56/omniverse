...
export default function NewEmailCampaignPage({ params: paramsPromise }) {
  ...
  const fetchTemplates = async () => {
    try {
      const res = await fetch("/api/templates?platform=email");
  ...
  const addFollowUp = () => {
    setFollowUps([...followUps, { delayDays: 3, subject: "Re: " + (subject || "Follow up"), message: "" }]);
  };
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!name || !listId || !message || !subject) {
  ...
  if (loading) return (
    <div className="h-screen w-full flex flex-col items-center justify-center bg-gray-50/50 space-y-6">
       ...
       <p className="text-gray-400 font-black uppercase tracking-[0.2em] text-[10px]">Testing SMTP Handshakes...</p>
    </div>
  );
    ...
    <div className="w-full min-h-screen bg-gray-50/50 p-4 md:p-8 animate-in fade-in duration-500">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="flex items-center gap-4 mb-10">
          <Link href={`/email/${accountId}/campaigns`} className="p-3 bg-white border border-gray-100 rounded-2xl text-gray-400 hover:text-gray-900 transition-all shadow-sm">
            <ChevronLeft size={20} />
          </Link>
          <div className="h-10 w-px bg-gray-200 mx-2 hidden md:block" />
          <div className="min-w-0">
            <div className="flex items-center gap-2 mb-1">
               <span className="px-2 py-0.5 bg-indigo-50 text-indigo-600 text-[9px] font-black uppercase tracking-widest rounded-full border border-indigo-100">Mail Protocol V2</span>
            </div>
            <h1 className="text-2xl md:text-3xl font-black text-gray-900 tracking-tight truncate">Construct Email Sequence</h1>
            <p className="text-gray-500 text-xs md:text-sm font-medium mt-1">Routing via SMTP node <span className="text-indigo-600 font-bold">{account?.email}</span></p>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-10">
          ...
          <div className="bg-white rounded-[2.5rem] border border-gray-100 shadow-xl shadow-gray-200/20 overflow-hidden">
            ...
            <div className="p-8 md:p-10 grid grid-cols-1 md:grid-cols-2 gap-8">
              ...
              <div className="space-y-3">
                <label className="text-[10px] font-black text-gray-400 uppercase tracking-[0.2em] ml-1">Lead Population</label>
                <select
                  value={listId}
          ...
          <div className="bg-white rounded-[2.5rem] border border-gray-100 shadow-xl shadow-gray-200/20 overflow-hidden">
             <div className="p-6 md:p-8 border-b border-gray-50 flex items-center gap-4 bg-gray-50/30">
              ...
              <div>
                <h2 className="text-lg font-black text-gray-900 tracking-tight">Core Payload</h2>
                <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Email subject & body logic</p>
              </div>
            </div>
            <div className="p-8 md:p-10 space-y-8">
               <div className="space-y-4">
                  <div className="flex items-center justify-between ml-1">
                    <label className="text-[10px] font-black text-gray-400 uppercase tracking-[0.2em]">Subject Line</label>
                    <div className="flex items-center gap-2">
                       {["$$f_name$$", "$$company$$"].map(v => (
                        <button key={v} type="button" onClick={() => insertVariable("subject", v)} className="px-2 py-0.5 bg-gray-50 text-indigo-600 text-[9px] font-black border border-gray-100 rounded hover:border-indigo-200 transition-all">{v}</button>
                      ))}
                    </div>
                  </div>
                  <input
                    ...
                    placeholder="Quick question for $$f_name$$"
                    className="w-full bg-gray-50 border border-gray-100 rounded-2xl px-6 py-4 outline-none focus:ring-4 focus:ring-indigo-500/10 transition-all font-bold text-gray-900"
                    required
                  />
               </div>

               <div className="pt-6 border-t border-gray-50 flex flex-col md:flex-row gap-6 items-start md:items-center">
                 ...
                 {!showSaveTemplate ? (
                    <button
                      ...
                      className="whitespace-nowrap flex items-center gap-2 text-indigo-600 font-black text-[10px] uppercase tracking-widest hover:bg-indigo-50 px-6 py-4 rounded-2xl border border-dashed border-indigo-200 transition-all mt-6 md:mt-7"
                    >
                      <Plus size={16} /> Save as New Template
                    </button>
                 ) : (
                    <div className="flex-1 w-full p-6 bg-indigo-50/30 rounded-3xl border border-indigo-50 flex flex-col gap-4 shadow-inner">
                       <input
                        type="text"
                        value={templateName}
                        onChange={(e) => setTemplateName(e.target.value)}
                        placeholder="Template Identifier"
                        className="w-full bg-white border border-indigo-100 rounded-xl px-4 py-2 text-sm font-bold outline-none"
                      />
                      <div className="flex gap-2">
                        <button onClick={handleSaveAsTemplate} className="flex-1 bg-indigo-600 text-white text-[9px] font-black uppercase py-2 rounded-lg">Construct Template</button>
               ...
               <div className="space-y-4">
                <div className="flex items-center justify-between">
                   <label className="text-[10px] font-black text-gray-400 uppercase tracking-[0.2em] ml-1">Email Body Payload</label>
                   <div className="flex items-center gap-2">
                      {["$$f_name$$", "$$full_name$$", "$$company$$", "$$website$$"].map(v => (
                        <button
                          key={v}
                          type="button"
                          onClick={() => insertVariable("main", v)}
                          className="px-3 py-1 bg-white border border-gray-100 rounded-lg text-[9px] font-black text-indigo-600 hover:border-indigo-200 transition-all"
                        >
                          {v}
                        </button>
                      ))}
                ...
                <textarea
                  ...
                  placeholder="Draft your personalized cold email sequence..."
                  rows={8}
          ...
          <div className="bg-white rounded-[2.5rem] border border-gray-100 shadow-xl shadow-gray-200/20 overflow-hidden">
            <div className="p-6 md:p-8 border-b border-gray-50 flex items-center gap-4 bg-gray-50/30">
              ...
              <div>
                ...
                <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Daily caps & sending logic</p>
              </div>
            </div>
            <div className="p-8 md:p-10 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
              <div className="space-y-3">
                <label className="text-[10px] font-black text-gray-400 uppercase tracking-[0.2em] ml-1">Daily Traffic Cap</label>
                <div className="relative">
                  ...
                  />
                  <span className="absolute right-4 top-1/2 -translate-y-1/2 text-[9px] font-black text-indigo-500 uppercase">Limit: 500/Day</span>
                </div>
              </div>
              <div className="space-y-3">
                <label className="text-[10px] font-black text-gray-400 uppercase tracking-[0.2em] ml-1">Send Latency (Min)</label>
                <div className="flex items-center gap-3">
                  <input type="number" min="1" value={minDelay} onChange={(e) => setMinDelay(e.target.value)} className="w-full bg-gray-50 border border-gray-100 rounded-xl p-3 text-center font-bold" />
                  <span className="text-gray-300 font-black">{"->"}</span>
                  <input type="number" max="60" value={maxDelay} onChange={(e) => setMaxDelay(e.target.value)} className="w-full bg-gray-50 border border-gray-100 rounded-xl p-3 text-center font-bold" />
                </div>
              </div>
               <div className="space-y-3">
            ...
            <div className="p-6 md:p-8 border-b border-gray-50 flex items-center justify-between bg-gray-50/30">
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Multi-step email follow-up logic</p>
                </div>
              </div>
              <button
                type="button"
              ...
              >
                <Plus size={16} /> Stack Email Node
              </button>
            </div>
            <div className="p-8 md:p-10 space-y-8">
              {followUps.length === 0 ? (
                <div className="text-center py-10 opacity-30 italic font-medium text-gray-400 text-sm">
                  Linear outreach. No follow-up emails scheduled.
                </div>
              ) : (
                <div className="space-y-6">
                  {followUps.map((step, idx) => (
                    <div key={idx} className="bg-gray-50/50 rounded-3xl border border-gray-100 p-8 relative group hover:bg-white hover:border-indigo-100 transition-all shadow-sm">
                       <button
                        type="button"
                      ...
                      <div className="flex flex-col md:flex-row gap-6 mb-6">
                        <div className="flex items-center gap-4">
                           ...
                           <div className="flex items-center gap-2">
                             ...
                             <input
                                ...
                                className="w-16 bg-white border border-gray-200 rounded-lg py-1 text-center font-black text-sm outline-none"
                              />
                        ...
                        <div className="flex-1">
                           <input
                              ...
                              className="w-full bg-white border border-gray-100 rounded-xl px-4 py-2 text-sm font-bold outline-none focus:border-indigo-300 transition-all"
                           />
                        </div>
                      </div>
                      <textarea
                        ...
                        placeholder="Configure layer email payload..."
                        rows={3}
          ...
          <div className="bg-[#111827] rounded-[2.5rem] border border-gray-800 shadow-2xl p-8 md:p-10 flex flex-col md:flex-row items-center justify-between gap-10">
             <div className="flex-1">
                ...
                <div className="flex items-center justify-between p-6 bg-gray-800/50 rounded-3xl border border-gray-700">
                   <div className="pr-4 text-left">
                      <p className="text-sm font-bold text-gray-200">Halt sequence on Reply</p>
                      <p className="text-[10px] text-gray-400 mt-1 uppercase font-black">Pause all routing if engagement is detected</p>
                   </div>
                   <label className="relative inline-flex items-center cursor-pointer">
                    ...
                    />
                    <div className="w-14 h-8 bg-gray-700 peer-focus:outline-none rounded-full peer peer-checked:after:translate-x-6 after:content-[''] after:absolute after:top-[4px] after:left-[4px] after:bg-white after:rounded-full after:h-6 after:w-6 after:transition-all peer-checked:bg-indigo-600 border border-gray-600 transition-all shadow-inner"></div>
                  </label>
                </div>
             </div>

             <div className="w-full md:w-auto flex flex-col gap-4 shrink-0 text-left">
                <button
                  type="submit"
                  disabled={submitting}
                ...
                >
                  {submitting ? "Initializing Sequence..." : "Deploy Outreach Stream"}
                  <Mail size={18} />
                </button>
                <button
                  type="button"
                  onClick={() => router.back()}
                  className="px-12 py-4 bg-gray-800 text-gray-400 font-black text-[10px] uppercase tracking-[0.2em] rounded-2xl hover:bg-gray-700 transition-all flex items-center justify-center border border-gray-700/50"
                >
                  Abort Construction
                </button>
             </div>
