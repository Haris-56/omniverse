...
async function processCampaignJob(job) {
    ...
    try {
        ...
        let proxyAgent = null;
        let trackedProxyId = null;

        let assignedProxy = null;
        if (selectedAccount.proxy && selectedAccount.proxy.host) {
            // User explicitly bound a proxy to this specific account
            const p = selectedAccount.proxy;
            const authStr = p.username ? `${encodeURIComponent(p.username)}:${encodeURIComponent(p.password)}@` : '';
            const proxyUrl = `${p.protocol}://${authStr}${p.host}:${p.port}`;
            proxyAgent = p.protocol.startsWith('socks') ? new SocksProxyAgent(proxyUrl) : new HttpsProxyAgent(proxyUrl);
            console.log(`[Proxy] Using account-bound proxy ${p.host} for ${selectedAccount.email}.`);
            // Set assignedProxy to a dummy or the proxy object to satisfy the check below
            assignedProxy = { ...p, _id: p._id || 'account-bound' }; 
        } else {
            // --- DYNAMIC IP GOVERNANCE ENGINE ---
            // Requirement: No proxy exceeding 1000 emails per day
            const startOfDay = new Date(new Date().setUTCHours(0, 0, 0, 0));

            if (campaign.assignedProxyId) {
                assignedProxy = await db.collection('system_proxies').findOne({ _id: new ObjectId(campaign.assignedProxyId) });
                if (assignedProxy) {
                    const todayUsage = await db.collection('email_logs').countDocuments({
                        proxyId: new ObjectId(campaign.assignedProxyId),
                        sentAt: { $gte: startOfDay }
                    });
                    if (todayUsage >= 1000) {
                        console.log(`[Proxy] Proxy ${assignedProxy.host} reached 1000/day limit. Unassigning from campaign ${campaign.name}.`);
                        assignedProxy = null; // Forces recalculation below
                    }
                }
            }

            if (!assignedProxy) {
                const availableProxies = await db.collection('system_proxies').find({ status: 'active' }).toArray();
                if (availableProxies.length > 0) {
                    // Fetch global proxy usage
                    const usageAgg = await db.collection('email_logs').aggregate([
                        { $match: { sentAt: { $gte: startOfDay }, proxyId: { $ne: null } } },
                        { $group: { _id: "$proxyId", count: { $sum: 1 } } }
                    ]).toArray();

                    const usageMap = {};
                    usageAgg.forEach(u => { if (u._id) usageMap[u._id.toString()] = u.count; });

                    // Find proxies strictly under 1000
                    const validProxies = availableProxies.filter(p => (usageMap[p._id.toString()] || 0) < 1000);

                    if (validProxies.length > 0) {
                        // Sort by lowest usage to perfectly distribute volume across all proxies
                        validProxies.sort((a, b) => (usageMap[a._id.toString()] || 0) - (usageMap[b._id.toString()] || 0));
                        assignedProxy = validProxies[0];
                        
                        await db.collection('email_campaigns').updateOne(
                            { _id: campaign._id },
                            { $set: { assignedProxyId: assignedProxy._id.toString() } }
                        );
                        console.log(`[Proxy] Assigned new IP ${assignedProxy.host} to campaign ${campaign.name}.`);
                    } else {
                        console.log(`[Proxy] ALL system proxies have maxed out 1000/day limit. Pausing campaign ${campaign.name} for 2 hours.`);
                        await db.collection('email_campaigns').updateOne({ _id: campaign._id }, { $set: { nextRunAt: new Date(Date.now() + 7200000) } });
                        return;
                    }
                }
            }
            ...
            if (assignedProxy) {
                const p = assignedProxy;
                trackedProxyId = ObjectId.isValid(p._id) ? new ObjectId(p._id) : null;
                
                // Handle different auth structures (system_proxies vs account-bound)
                const username = p.auth?.username || p.username;
                const password = p.auth?.password || p.password;
                const authStr = username ? `${encodeURIComponent(username)}:${encodeURIComponent(password)}@` : '';
                
                const proxyUrl = `${p.protocol}://${authStr}${p.host}:${p.port}`;
                proxyAgent = p.protocol.startsWith('socks') ? new SocksProxyAgent(proxyUrl) : new HttpsProxyAgent(proxyUrl);
                console.log(`[Proxy] Routing through proxy ${p.host} for campaign ${campaign.name}.`);
            } else {
                console.error(`[CRITICAL] DigitalOcean port blocking detected. No proxies available in 'system_proxies' or account settings.`);
                console.error(`Please add active proxies to 'system_proxies' or bind one to account ${selectedAccount.email}.`);
                await db.collection('email_campaigns').updateOne(
                    { _id: campaign._id }, 
                    { $set: { status: 'Paused', nextRunAt: new Date(Date.now() + 3600000) } } 
                );
                await logSystemEvent('ERROR', 'email-worker', `Campaign ${campaign.name} paused: Proxy required on DigitalOcean but none found.`);
                return;
            }
        }
        ...
        try {
            ...
            await db.collection('email_logs').insertOne({
                ...
                campaignId: campaign._id,
                contactId: targetContact._id,
                proxyId: trackedProxyId, // Critical for tracking 1000/day Governance
                sentAt: new Date(),
