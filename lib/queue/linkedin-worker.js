...
async function processCampaignJob(job) {
    ...
    try {
        ...
        // Run AI Closer inbox processor
        try {
            const { runInboxCloserSession } = await import('../social-automation/closer-processor.js');
            await runInboxCloserSession(db, 'linkedin', account, page, campaign);
        } catch (closerErr) {
            console.error("[LinkedIn Worker] Inbox Closer Error:", closerErr);
        }
