...
export async function executeOutreach(page, profileTarget, message, options = {}) {
    try {
        ...
        if (!targetUrl.includes('facebook.com')) {
            targetUrl = `https://www.facebook.com/${targetUrl.replace('@', '')}`;
        } else if (!targetUrl.startsWith('http')) {
            targetUrl = `https://${targetUrl}`;
        }
        
        console.log(`[FB Outreach] Navigating to ${targetUrl}`);
        await page.goto(targetUrl, { waitUntil: 'load', timeout: 60000 });
        
        // Stealth reading delay
        ...
        if (msgBtn) {
            await msgBtn.click();
            await Humanoid.sleep(8000, 20000); // Wait for chat overlay

            const dmInput = await page.$('div[role="textbox"][aria-label="Message"]');
            if (dmInput && message) {
                await dmInput.focus();
                await Humanoid.sleep(2000, 5000);
                await page.keyboard.type(message, { delay: Math.floor(Math.random() * 80) + 40 });
                await Humanoid.sleep(4000, 10000);
                await page.keyboard.press('Enter');
                
                await Humanoid.sleep(5000, 10000);
                return { success: true, method: 'facebook_dm' };
            } else {
                return { success: false, reason: 'Chat window opened but input not found' };
            }
        } else {
            return { success: false, reason: 'Message button not found on profile' };
        }
    } catch (e) {
        console.error("[FB Outreach Error]", e.message);
