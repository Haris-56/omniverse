  ...
  try {
    ...
    if (await checkLoggedIn()) {
        ...
        try {
            ...
            await Promise.race([
                page.waitForURL(/.*linkedin\.com.*/, { waitUntil: 'load', timeout: 45000 }),
                page.waitForSelector('.error-for-password, #error-for-username', { timeout: 20000 }).catch(() => null)
            ]);

            const errorMsg = await page.$eval('.error-for-password, #error-for-username', el => el.innerText).catch(() => null);
            if (errorMsg) throw new Error(`LinkedIn says: ${errorMsg}`);
