    ...
    try {
        ...
        for (const method of priorityOrder) {
            ...
            if (method === 'story') {
                ...
                // Target highlight section which is below the main header but above the posts.
                // Safest way is to restrict text or structure, but for highlights:
                ...
                if (messageBtn) {
                    ...
                    try {
                        await page.waitForSelector('div[role="textbox"][aria-label="Message"], textarea[placeholder*="Message"], div[contenteditable="true"]', { timeout: 15000 });
                    } catch(e) {
                        console.log(`[Outreach] DM input selector wait manually ignored...`);
                    }

                    let dmInput = null;
                    const inputs = await page.$$('div[role="textbox"][aria-label="Message"], textarea[placeholder*="Message"], div[contenteditable="true"]');
