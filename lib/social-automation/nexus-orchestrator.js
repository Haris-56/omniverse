...
export async function processNexusCampaigns(db) {
    ...
    for (const campaign of campaigns) {
        ...
        if (!campaign.listId) {
             console.log(`  -> Campaign ${campaign.name} has no listId. Skipped.`);
             continue;
        }
                    ...
                    if (account) {
                        ...
                        if (target) {
                             ...
                             if (session && session.success) {
                                 // RUN INBOX CLOSER DMs SCAN & REPLY
                                 try {
                                     const { runInboxCloserSession } = await import('./closer-processor.js');
                                     await runInboxCloserSession(db, 'instagram', account, session.page, campaign);
                                 } catch (closerErr) {
                                     console.error("  -> Nexus IG Inbox Closer Error:", closerErr);
                                 }
                    ...
                    if (account) {
                        ...
                        if (target) {
                             ...
                             if (session && session.success) {
                                 ...
                                 try {
                                     const { runInboxCloserSession } = await import('./closer-processor.js');
                                     await runInboxCloserSession(db, 'facebook', account, session.page, campaign);
                                 } catch (closerErr) {
                                     console.error("  -> Nexus FB Inbox Closer Error:", closerErr);
                                 }
